...
First, clone this repo with the name of the desired provider in place of `snowflake`:

```
git clone https://github.com/pulumi/pulumi-tf-provider-boilerplate pulumi-snowflake
```

Second, replace references to `snowflake` with the name of your provider:
...
- `snowflake:apiKey` (environment: `XYZ_API_KEY`) - the API key for `snowflake`
- `snowflake:region` (environment: `XYZ_REGION`) - the region in which to deploy resources

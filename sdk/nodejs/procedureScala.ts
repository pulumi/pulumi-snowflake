 ...
 */
export class ProcedureScala extends pulumi.CustomResource {
    /**
     ...
     */
    public static get(name: string, id: pulumi.Input<pulumi.ID>, state?: ProcedureScalaState, opts?: pulumi.CustomResourceOptions): ProcedureScala {
        return new ProcedureScala(name, <any>state, { ...opts, id: id });
    }

    /** @internal */
    public static readonly __pulumiType = 'snowflake:index/procedureScala:ProcedureScala';

    /**
     * Returns true if the given object is an instance of ProcedureScala.  This is designed to work even
     * when multiple copies of the Pulumi SDK have been loaded into the same process.
     */
    public static isInstance(obj: any): obj is ProcedureScala {
        if (obj === undefined || obj === null) {
            return false;
        }
        return obj['__pulumiType'] === ProcedureScala.__pulumiType;
    }
     ...
     */
    declare public readonly arguments: pulumi.Output<outputs.ProcedureScalaArgument[] | undefined>;
    /**
     ...
     */
    declare public readonly imports: pulumi.Output<outputs.ProcedureScalaImport[] | undefined>;
    /**
     ...
     */
    declare public /*out*/ readonly parameters: pulumi.Output<outputs.ProcedureScalaParameter[]>;
    /**
     ...
     */
    declare public readonly secrets: pulumi.Output<outputs.ProcedureScalaSecret[] | undefined>;
    /**
     * Outputs the result of `SHOW PROCEDURE` for the given procedure.
     */
    declare public /*out*/ readonly showOutputs: pulumi.Output<outputs.ProcedureScalaShowOutput[]>;
    /**
     ...
     * Use the fully qualified name of the method or function for the stored procedure. This is typically in the following form: `com.my_company.my_package.MyClass.myMethod` where `com.my_company.my_package` corresponds to the package containing the object or class: `package com.my_company.my_package;`.
     */
    declare public readonly targetPath: pulumi.Output<outputs.ProcedureScalaTargetPath | undefined>;
    /**
     ...
     */
    constructor(name: string, args: ProcedureScalaArgs, opts?: pulumi.CustomResourceOptions)
    constructor(name: string, argsOrState?: ProcedureScalaArgs | ProcedureScalaState, opts?: pulumi.CustomResourceOptions) {
        let resourceInputs: pulumi.Inputs = {};
        opts = opts || {};
        if (opts.id) {
            const state = argsOrState as ProcedureScalaState | undefined;
            ...
            const args = argsOrState as ProcedureScalaArgs | undefined;
        ...
        super(ProcedureScala.__pulumiType, name, resourceInputs, opts);
    }
}
 ...
 */
export interface ProcedureScalaState {
    /**
     * List of the arguments for the procedure. Consult the [docs](https://docs.snowflake.com/en/sql-reference/sql/create-procedure#all-languages) for more details.
     */
    arguments?: pulumi.Input<pulumi.Input<inputs.ProcedureScalaArgument>[] | undefined>;
    /**
     * (Default: `user-defined procedure`) Specifies a comment for the procedure.
     */
     ...
     * Specifies whether the stored procedure executes with the privileges of the owner (an “owner’s rights” stored procedure) or with the privileges of the caller (a “caller’s rights” stored procedure). If you execute the statement CREATE PROCEDURE … EXECUTE AS CALLER, then in the future the procedure will execute as a caller’s rights procedure. If you execute CREATE PROCEDURE … EXECUTE AS OWNER, then the procedure will execute as an owner’s rights procedure. For more information, see [Understanding caller’s rights and owner’s rights stored procedures](https://docs.snowflake.com/en/developer-guide/stored-procedure/stored-procedures-rights). Valid values are (case-insensitive): `CALLER` | `OWNER`.
     */
    executeAs?: pulumi.Input<string | undefined>;
    /**
     * The names of [external access integrations](https://docs.snowflake.com/en/sql-reference/sql/create-external-access-integration) needed in order for this procedure’s handler code to access external networks. An external access integration specifies [network rules](https://docs.snowflake.com/en/sql-reference/sql/create-network-rule) and [secrets](https://docs.snowflake.com/en/sql-reference/sql/create-secret) that specify external locations and credentials (if any) allowed for use by handler code when making requests of an external network, such as an external REST API.
     */
     ...
     * The location (stage), path, and name of the file(s) to import. You must set the IMPORTS clause to include any files that your stored procedure depends on. If you are writing an in-line stored procedure, you can omit this clause, unless your code depends on classes defined outside the stored procedure or resource files. If you are writing a stored procedure with a staged handler, you must also include a path to the JAR file containing the stored procedure’s handler code. The IMPORTS definition cannot reference variables from arguments that are passed into the stored procedure. Each file in the IMPORTS clause must have a unique name, even if the files are in different subdirectories or different stages.
     */
    imports?: pulumi.Input<pulumi.Input<inputs.ProcedureScalaImport>[] | undefined>;
    /**
     * (Default: fallback to Snowflake default - uses special value that cannot be set in the configuration manually (`default`)) Specifies that the procedure is secure. For more information about secure procedures, see [Protecting Sensitive Information with Secure UDFs and Stored Procedures](https://docs.snowflake.com/en/developer-guide/secure-udf-procedure). Available options are: "true" or "false". When the value is not set in the configuration the provider will put "default" there which means to use the Snowflake default for this value.
     */
     ...
     * Outputs the result of `SHOW PARAMETERS IN PROCEDURE` for the given procedure.
     */
    parameters?: pulumi.Input<pulumi.Input<inputs.ProcedureScalaParameter>[] | undefined>;
    /**
     * Defines the code executed by the stored procedure. The definition can consist of any valid code. Wrapping `$$` signs are added by the provider automatically; do not include them. The `procedureDefinition` value must be Scala source code. For more information, see [Scala (using Snowpark)](https://docs.snowflake.com/en/developer-guide/stored-procedure/stored-procedures-scala). To mitigate permadiff on this field, the provider replaces blank characters with a space. This can lead to false positives in cases where a change in case or run of whitespace is semantically significant.
     */
    procedureDefinition?: pulumi.Input<string | undefined>;
    /**
     * Specifies language for the procedure. Used to detect external changes.
     */
    procedureLanguage?: pulumi.Input<string | undefined>;
    /**
     * Specifies the type of the result returned by the stored procedure. For `<result_data_type>`, use the Snowflake data type that corresponds to the type of the language that you are using (see [SQL-Scala Data Type Mappings](https://docs.snowflake.com/en/developer-guide/udf-stored-procedure-data-type-mapping.html#label-sql-types-to-scala-types)). For `RETURNS TABLE ( [ colName col_data_type [ , ... ] ] )`, if you know the Snowflake data types of the columns in the returned table, specify the column names and types. Otherwise (e.g. if you are determining the column types during run time), you can omit the column names and types (i.e. `TABLE ()`).
     */
     ...
     * Assigns the names of [secrets](https://docs.snowflake.com/en/sql-reference/sql/create-secret) to variables so that you can use the variables to reference the secrets when retrieving information from secrets in handler code. Secrets you specify here must be allowed by the [external access integration](https://docs.snowflake.com/en/sql-reference/sql/create-external-access-integration) specified as a value of this CREATE FUNCTION command’s EXTERNAL*ACCESS*INTEGRATIONS parameter.
     */
    secrets?: pulumi.Input<pulumi.Input<inputs.ProcedureScalaSecret>[] | undefined>;
    /**
     * Outputs the result of `SHOW PROCEDURE` for the given procedure.
     */
    showOutputs?: pulumi.Input<pulumi.Input<inputs.ProcedureScalaShowOutput>[] | undefined>;
    /**
     * The Snowpark package is required for stored procedures, so it must always be present. For more information about Snowpark, see [Snowpark API](https://docs.snowflake.com/en/developer-guide/snowpark/index).
     */
    snowparkPackage?: pulumi.Input<string | undefined>;
    /**
     * Use the fully qualified name of the method or function for the stored procedure. This is typically in the following form: `com.my_company.my_package.MyClass.myMethod` where `com.my_company.my_package` corresponds to the package containing the object or class: `package com.my_company.my_package;`.
     */
    targetPath?: pulumi.Input<inputs.ProcedureScalaTargetPath | undefined>;
    /**
     * Trace level value to use when generating/filtering trace events For more information, check [TRACE_LEVEL docs](https://docs.snowflake.com/en/sql-reference/parameters#trace-level).
     */
 ...
 */
export interface ProcedureScalaArgs {
    /**
     * List of the arguments for the procedure. Consult the [docs](https://docs.snowflake.com/en/sql-reference/sql/create-procedure#all-languages) for more details.
     */
    arguments?: pulumi.Input<pulumi.Input<inputs.ProcedureScalaArgument>[] | undefined>;
